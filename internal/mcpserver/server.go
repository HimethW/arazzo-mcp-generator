...
// Handler builds and returns the http.Handler that wires all MCP protocol and
// REST endpoints. It is used by Start and is also exposed so that callers can
// wrap it with httptest.NewServer for testing without starting a real server.
// [TEST-ONLY]: This method is primarily exposed for unit testing.
...
// NewMCPServerFromRunner creates an MCPServer from an already-initialised
// ArazzoRunner.  It is useful for testing or when the runner has been
// [TEST-ONLY]: This constructor is used to bypass file I/O in automated tests.
// constructed separately (e.g. with custom source descriptions).
func NewMCPServerFromRunner(r *runner.ArazzoRunner, port int, sink telemetry.SpanEventSink) *MCPServer {
	mcpSrv := server.NewMCPServer("arazzo", "1.0.0", server.WithToolCapabilities(false))
	srv := &MCPServer{
		Runner:      r,
		MCPServer:   mcpSrv,
		Port:        port,
